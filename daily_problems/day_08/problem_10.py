...
def prediction_summary(predictions: List[Dict]) -> Dict[str, float]:
    ...
    for prediction in predictions:
        ...
        if prediction["score"] >= 0.7:
            # threshold_crossers.append(prediction["score"])
            above_threshold += 1    
        all_scores.append(prediction["score"])
    avg_score = sum(all_scores)/len(all_scores)
        # print("Threshold_crossers: ", threshold_crossers)
    return {"total_predictions": len(predictions), "above_threshold": above_threshold, "avg_score": avg_score}
